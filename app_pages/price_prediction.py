...
def app():
    ...
    model = joblib.load('models/house_price_model_xgb.pkl')

    # Load inherited house predictions
    inherited_preds = pd.read_csv('data/predictions/inherited_houses_predictions.csv')
    st.subheader("Predicted Prices for Inherited Houses")
    st.dataframe(inherited_preds)
    st.success(f"Total Expected Sale Value: ${inherited_preds['Predicted_SalePrice'].sum():,.2f}")
    ...
    if st.button("Predict Price"):
        ...
        train_data = pd.read_csv('data/processed/cleaned_housing_data.csv')
        feature_list = train_data.select_dtypes(include='number').drop(columns=['SalePrice']).columns.tolist()
